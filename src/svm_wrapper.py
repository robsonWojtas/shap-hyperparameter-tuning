...
class SVMWrapper:
    ...
    def __init__(self, x_train, y_train, x_val, y_val, param_names, int_params, kernel_dict):
        print('init SVMWrapper')
    ...
    def params_to_dict(self, row):
        ...
        params_dict['kernel'] = self.get_kernel(params_dict['kernel'])
        return params_dict
...
if __name__ == '__main__':
    ...
    # #     PARAMS PREPARATION     ################################
    # #############################################################
    # columns = ['tol', 'max_iter', 'C', 'coef0', 'degree', 'kernel']
    # tol = [0.0001, 0.001, 0.1, 0.2]
    # max_iter = [200, 300, 400, 500]
    # C = [0.1, 1, 3, 5]
    # coef0 = [0, 0.1, 1, 3]
    # degree = [1, 2, 3, 6]
    # kernel = list(range(1, 2))
    # kernel_dict = {
    #     1: 'sigmoid',
    #     # 1: 'linear',
    #     # 2: 'poly',
    ...
    #     # 4: 'sigmoid'
    # }
    # int_columns = ['max_iter', 'degree', 'kernel']
    # #############################################################
    # #     EXPERIMENTS - SINGLE RUN     ##########################
    # #############################################################
    # train_X, val_X, train_y, val_y = train_test_split(X, Y, test_size=0.3, random_state=0)
    #
    # wrapper = SVMWrapper(x_train=train_X, y_train=train_y, x_val=val_X, y_val=val_y,
    #                      param_names=columns, int_params=int_columns, kernel_dict=kernel_dict)
    #
    # input_data = get_params_dataset([tol, max_iter, C, coef0, degree, kernel], columns)
    ...
    # shap_values = explainer(input_data[0:10])
    #
    # #############################################################
    # #     RESULT PLOT     #######################################
    # #############################################################
    ...
    train_X, val_X, train_y, val_y = train_test_split(X_iris, Y_iris, test_size=0.3, random_state=0)
    wrapper_iris = SVMWrapper(x_train=train_X, y_train=train_y, x_val=val_X, y_val=val_y,
                              param_names=columns, int_params=int_columns, kernel_dict=kernel_dict)
    ...
    train_X, val_X, train_y, val_y = train_test_split(X_wine, Y_wine, test_size=0.3, random_state=0)
    wrapper_wine = SVMWrapper(x_train=train_X, y_train=train_y, x_val=val_X, y_val=val_y,
                              param_names=columns, int_params=int_columns, kernel_dict=kernel_dict)
    ...
    train_X, val_X, train_y, val_y = train_test_split(X_diabetes, Y_diabetes, test_size=0.3, random_state=0)
    wrapper_diabetes = SVMWrapper(x_train=train_X, y_train=train_y, x_val=val_X, y_val=val_y,
                                  param_names=columns, int_params=int_columns, kernel_dict=kernel_dict)
    ...
    train_X, val_X, train_y, val_y = train_test_split(X_bc, Y_bc, test_size=0.3, random_state=0)
    wrapper_bc = SVMWrapper(x_train=train_X, y_train=train_y, x_val=val_X, y_val=val_y,
                            param_names=columns, int_params=int_columns, kernel_dict=kernel_dict)
    ...
    shap_values_std_df.drop(labels=['kernel'], axis=1, inplace=True)
    ...
    #############################################################
    # with open('.\src\masters_utils\workspaces\svm_linear_ranking.pkl', 'wb') as file:
    #     pickle.dump(
    #         (shap_values_iris, shap_values_wine, shap_values_diabetes, shap_values_bc, shap_values_std_df, rank), file)
    # with open('.\src\masters_utils\workspaces\svm_linear_ranking.pkl', 'rb') as file:
    #     shap_values_iris, shap_values_wine, shap_values_diabetes, shap_values_bc, shap_values_std_df, rank = pickle.load(
    #         file)